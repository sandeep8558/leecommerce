...
<div class="container-fluid shadow">
    <div class="container py-2 submenus text-center">
        ...
        <div class="btn-group">
            ...
            <a href="/category/{{$category->id}}/sub_category/{{$sub->id}}#myButton_{{$sub->id}}" class="btn btn-outline-dark ButtonLink px-5 {{ (request()->is('category/'.$category->id.'/sub_category/'.$sub->id)) ? 'active' : '' }}" id="myButton_{{$sub->id}}" aria-current="page">{{$sub->sub_category}}</a>
            @endforeach
        </div>
        @endif
